...
def xgboost(symptom1, symptom2, symptom3, symptom4, symptom5):
...
def mainFunc():
    ...
    if st.button("Submit"):
        ...
        predicted_disease = xgboost(symptom1, symptom2, symptom3, symptom4, symptom5)
    ...
    for i in l:
        i_parts = i.split(":", 1)
        search_term = i_parts[0].split(".")[1].strip()  # Extract the term after the number and dot
        searchUrl = urlencode({"q": search_term})
        st.markdown(f"{i}(https://www.google.com/search?{searchUrl})")
